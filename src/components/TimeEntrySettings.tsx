          ...
          {(settings.taxTypes || []).map(tax => <div key={tax.id} className="flex items-center justify-between min-h-[20px]">
              <div className="flex-1 flex items-center">
                {editingTax?.id === tax.id ? (
                  ...
                  <span className="text-[#09121F] text-sm flex-1 leading-5">{tax.name}</span>
                )}
              </div>
              <div className="flex items-center gap-3">
                {settings.invoiceMode && (
                  <div className="min-w-[60px] text-right">
                    {editingTax?.id === tax.id ? (
                      <input 
                        type="text" 
                        value={editingTax.rate ? `${editingTax.rate}%` : ''}
                        onChange={(e) => {
                          const value = e.target.value.replace(/[^0-9.]/g, '');
                          setEditingTax({...editingTax, rate: parseFloat(value) || 0});
                        }}
                        onBlur={() => {
                          handleUpdateTax(editingTax);
                        }}

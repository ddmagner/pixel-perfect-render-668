...
export const TimeTally: React.FC = () => {
  ...
  return <div className="flex flex-col h-full w-full font-gilroy">
      ...
      <div className="w-full px-5">
        <div className={`grid ${gridColsWithSelection} h-[32px] items-center`} style={{ gridTemplateColumns: '32px 30% 30% 1fr' + (viewMode === 'invoice' ? ' 120px' : ''), gap: '0' }}>
            ...
            <div className="space-y-4">
              {organizedData.groups.map((group, groupIndex) => <div key={`${group.type}-${group.name}-${groupIndex}`}>
                  ...
                  {sortOption === 'project' && group.projects ? group.projects.map((project: any, projectIndex: number) => <div key={`project-${project.name}-${projectIndex}`}>
                        ...
                        {project.entries.map((entry: TimeEntry) => <div key={entry.id} className={`grid ${gridColsWithSelection} h-[32px] items-center hover:bg-gray-50`} style={{ gridTemplateColumns: '32px 30% 30% 1fr' + (viewMode === 'invoice' ? ' 120px' : ''), gap: '0' }}>
                            <div className="flex items-center w-[32px]">
                              <div 
                                className={`w-4 h-4 rounded-full border-2 border-gray-300 cursor-pointer flex items-center justify-center ${
                        ...
                        <div className="h-px bg-[#09121F]" />
                        <div className={`grid ${gridColsWithSelection} gap-0 h-[32px] items-center`} style={{ gridTemplateColumns: '32px 30% 30% 1fr' + (viewMode === 'invoice' ? ' 120px' : '') }}>
                          <div></div>
                          <div></div>
                      ...
                      {group.entries.map((entry: TimeEntry) => <div key={entry.id} className={`grid ${gridColsWithSelection} h-[32px] items-center hover:bg-gray-50`} style={{ gridTemplateColumns: '32px 30% 30% 1fr' + (viewMode === 'invoice' ? ' 120px' : ''), gap: '0' }}>
                            <div className="flex items-center w-[32px]">
                              <div 
                                className={`w-4 h-4 rounded-full border-2 border-gray-300 cursor-pointer flex items-center justify-center ${
                                  selection.isSelected(entry.id) ? 'bg-gray-300' : 'bg-white'

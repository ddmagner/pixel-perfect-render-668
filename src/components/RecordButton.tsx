...
// Reusable speech bubble SVG component with sanitized color
const SpeechBubble: React.FC<{ color: string }> = ({ color }) => {
  const safeColor = sanitizeColor(color);
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '12px', height: '12px', aspectRatio: '1/1', margin: '0 2px' }}>
      <path d="M5.75 0.5C8.78765 0.5 11.25 2.96235 11.25 6C11.25 9.03765 8.78765 11.5 5.75 11.5C4.8139 11.5 3.93225 11.2663 3.1606 10.8538L0.25 11.5L0.8968 8.5905C0.4843 7.8183 0.25 6.93665 0.25 6C0.25 2.96235 2.71235 0.5 5.75 0.5Z" fill={safeColor} />
    </svg>
  );
};
...
export const RecordButton: React.FC<RecordButtonProps> = ({
  ...
  // Sanitize the accent color for safe rendering
  const safeAccentColor = sanitizeColor(settings.accentColor);
  ...
  return (
    <section className="flex w-full justify-center items-center gap-2.5 px-0 pt-[30px] pb-[15px]">
      <div className="flex flex-col items-center gap-[25px] shrink-0">
        <div className="h-[220px] w-[220px] relative">
          <button
            className={`w-[220px] h-[220px] shrink-0 absolute left-0 top-0 transition-all flex items-center justify-center rounded-full select-none ${isPressed ? 'scale-95' : 'scale-100'} ${isRecording ? 'animate-pulse' : ''}`}
          ...
          >
            <svg width="220" height="221" viewBox="0 0 220 221" fill="none" xmlns="http://www.w3.org/2000/svg" className="record-button" style={{ width: '220px', height: '220px', flexShrink: 0 }}>
              <path d="M110 0.5C170.753 0.5 220 49.747 220 110.5C220 171.253 170.753 220.5 110 220.5C91.278 220.5 73.645 215.825 58.212 207.575L0 220.5L12.936 162.31C4.686 146.866 0 129.233 0 110.5C0 49.747 49.247 0.5 110 0.5Z" fill={isPressed ? '#BFBFBF' : safeAccentColor} />
        ...
        <div className="flex items-center justify-center whitespace-nowrap w-full">
          <div className="flex items-center">
            ...
            <SpeechBubble color={settings.accentColor} />
          </div>
